---
title: "Exploring trends and variation in Antibiotics"
...
We have prepared the following notebook for investigating the use of antibiotics in English hospitals. 
...
We will load the csv containing the SNOMED codes for the medicines we are investigating. It also contains relevant groupings and DDDs (where they exist).
...
#codes_coamox = codelist %>%
#  filter(ingredient == "Amoxicillin sodium") 

ingredients_allowed = codelist$ingredient %>%
  unique()
```

## Data

Before we can analyse trends and variation in the use of this group of medicines we need to prepare our dataset. 
...
dmd_check <- df_dmd_info %>%
  group_by(vmp_product_name) %>%
  mutate(ings = length(unique(strnt_nmrtr_val))) %>%
  ungroup() %>%
  filter(ings>1)

#write.csv(dmd_check,here("data/dmd_check.csv"))

# filter to ingredients that are antibiotics
df_dmd_info <- subset(df_dmd_info, ing_nm %in% ingredients_allowed) 

dmd_check2 <- df_dmd_info %>%
  group_by(vmp_product_name) %>%
  mutate(ings = length(unique(strnt_nmrtr_val))) %>%
  ungroup() %>%
  filter(ings>1)

#write.csv(dmd_check2,here("data/dmd_check2.csv"))
...
# note these ddds are from NHSD where there is only one value per vmp, hence we can ignore ingredients here (just need to drop duplicates)
...
# also add other groupings for each vmp
groups <- select(codelist, c('nm', 'Route', 'paragraph', 'category')) %>%
  unique()
...
#         & volume_ddd) %>%
  filter(substr(vmp_product_name, 1, 6) == 'Co-amo') %>%
  group_by(year_month) %>%
...
#write.csv(df_scmd_ddd,here("data/df_scmd_ddd.csv"))
```
...
#           as.Date("2020-04-01") & year_month <= as.Date("2020-08-01")
#         & volume_ddd) %>%
  filter(substr(vtmnm, 1, 6) == 'Co-amo') %>%
  group_by(vtmnm, year_month) %>%
  summarise(volume_ddd = sum(total_quantity, na.rm = TRUE)) 
...
```{r, fig.width=7.2, fig.height=3, cache=TRUE, fig.cap="Figure. National prescribing of antibiotics over time (filtered to top 20)."}

temp_ggplot <- df_scmd_ddd %>%
  group_by(year_month, vtmnm) %>%
  summarise(volume_ddd = sum(volume_ddd, na.rm = TRUE)) %>% 
  # now group to vtmnm to allow filtering to top 20 drugs
  group_by(vtmnm) %>%
  mutate(total = sum(volume_ddd, na.rm = TRUE)) %>%
  ungroup() %>% 
  mutate(rank = dense_rank(-total),
         vtmnm = fct_reorder(vtmnm, -rank)) %>%
  filter(rank <= 20) %>% 
  ...
  labs(x = NULL, y = "Defined Daily Dose (millions)",
       colour = NULL) +
...
### National prescribing by BNF Paragraph

```{r, fig.width=7.2, fig.height=3, cache=TRUE, fig.cap="Figure. National prescribing of antibiotics over time by BNF paragraph"}

temp_ggplot <- df_scmd_ddd %>%
  group_by(year_month, paragraph) %>%
  summarise(volume_ddd = sum(volume_ddd, na.rm = TRUE)) %>% 
  # now group to para to allow sorting
  group_by(paragraph) %>%
  mutate(total = sum(volume_ddd, na.rm = TRUE)) %>%
  ungroup() %>% 
  mutate(rank = dense_rank(-total),
         paragraph = fct_reorder(paragraph, rank)) %>%
  ggplot(aes(x = year_month, 
             y = volume_ddd/10^6, 
             colour = paragraph, 
             group = paragraph)) +
  ...
  geom_point(aes(text = paste0("<b>Month:</b> ", 
                               ...
                               lubridate::year(year_month), "<br>",
                               "<b>Volume:</b> ", round(volume_ddd, 0), "<br>",
                               "<b>Medication:</b> ", paragraph)), size = 2) +
  scale_x_date(date_breaks = "4 month", date_labels =  "%b %y") +  scale_colour_viridis_d() +
  labs(x = NULL, y = "Defined Daily Dose (millions)",
       colour = NULL) +
  geom_vline(xintercept = as.numeric(as.Date("2020-03-31")), 
...
### National prescribing by WHO Category

```{r, fig.width=7.2, fig.height=3, cache=TRUE, fig.cap="Figure. National prescribing of antibiotics over time by WHO Category"}
...
temp_ggplot <- df_scmd_ddd %>%
  ...
  geom_point(aes(text = paste0("<b>Month:</b> ", 
                               ...
                               lubridate::year(year_month), "<br>",
                               "<b>Volume:</b> ", round(volume_ddd, 0), "<br>",
                               "<b>Medication:</b> ", category)), size = 2) +
  scale_x_date(date_breaks = "4 month", date_labels =  "%b %y") +  scale_colour_viridis_d() +
  labs(x = NULL, y = "Defined Daily Dose (millions)",
       colour = NULL) +
  geom_vline(xintercept = as.numeric(as.Date("2020-03-31")), 
...
### National prescribing by Route of Administration

```{r, fig.width=7.2, fig.height=3, cache=TRUE, fig.cap="Figure. National prescribing of antibiotics over time by Route."}

temp_ggplot <- df_scmd_ddd %>%
  group_by(year_month, Route) %>%
  summarise(volume_ddd = sum(volume_ddd, na.rm = TRUE)) %>% 
  ggplot(aes(x = year_month, 
             y = volume_ddd/10^6, 
             colour = Route, 
...
```{r, fig.width=8, fig.height=8, fig.cap="Figure. Regional prescribing of Antibiotics over time, by BNF paragraph."}

temp_ggplot <- df_scmd_ddd_map %>%
  group_by(comm_region_name, year_month, paragraph) %>%
  summarise(volume_ddd = sum(volume_ddd, na.rm = TRUE)) %>% 
  mutate(comm_region_name = fct_explicit_na(comm_region_name)) %>% 
  # group to paragraph to allow sorting/filtering
  group_by(paragraph) %>%
  mutate(total = sum(volume_ddd, na.rm = TRUE)) %>%
  ungroup() %>% 
  mutate(rank = dense_rank(-total),
         paragraph = fct_reorder(paragraph, rank)) %>%
  filter(rank <= 5) %>% 
  ggplot(aes(x = year_month, y = volume_ddd/10^6, 
...
```{r fig.width=8, fig.height=4, fig.cap="Figure. Total regional prescribing of top 7 Antibiotics."}
temp_ggplot <- df_scmd_ddd_map %>% 
...
```{r, fig.width=8, fig.height=6, fig.cap="Figure. Total prescribing of Antibiotics for 20 STPs with the largest volume across all selected medications."}
