...
The file `antibac_codelist.csv` contains the SNOMED codes for the medicines we are investigating. It also contains relevant groupings and DDDs (where they exist).
...
groups <- select(codelist, c('nm', 'Route')) 
...
```{r, fig.width=8, fig.height=8, fig.cap="Figure. Regional prescribing of Antibiotics over time."}
...
temp_ggplot <- df_scmd_ddd_map %>%
  ...
  # group to vtmnm to allow filtering to top 20 drugs
  group_by(vtmnm) %>%
  drop_na('volume_ddd') %>%
  mutate(total = sum(volume_ddd)) %>%
  ungroup() %>% 
...
temp_ggplot <- df_scmd_ddd_map %>% 
  ...
  mutate(comm_region_name = fct_explicit_na(comm_region_name)) %>% 
  # now group to vtmnm to allow filtering to top 10 drugs
  group_by(vtmnm) %>%
  drop_na(volume_ddd) %>%
  mutate(total = sum(volume_ddd)) %>%
  ungroup() %>% 
