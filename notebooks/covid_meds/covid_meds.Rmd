...
df_etr_historic <- readr::read_csv(here::here("data/etr.csv"), col_names = FALSE) %>%
  select(1:4) 
...
stp_count <- left_join(stp_count, stp_to_region_map, on=c("stp_code"))
...
df_scmd_ddd_map_temp_temp <- df_scmd_ddd_map_temp %>% 
...
data_tab <- df_scmd_ddd_map_temp_temp %>% 
  mutate(prop_scale = seq(from = 0, to = 1, length = nrow(df_scmd_ddd_map_temp_temp)))
