...
temp_ggplot <- df_scmd_ddd %>%
  ...
  labs(x = NULL, y = "Defined Daily Dose (thousands)",
       colour = NULL) +
...
```{r fig.width=8, fig.height=4, fig.cap="Figure. Total regional prescribing of COVID treatments by region for financial year 2021-22."}
temp_ggplot <- df_scmd_ddd_map %>% 
  filter(year_month >= as.Date("2021-04-01") & year_month < as.Date("2022-04-01")) %>%
...
```{r, fig.width=8, fig.height=6, fig.cap="Figure. Total prescribing of COVID treatments in financial year 2021-22 for the top 20 STPs with the largest volume across all selected medications."}
...
create_med_use_table(data = df_scmd_ddd_map_temp, field = 'vtmnm')
