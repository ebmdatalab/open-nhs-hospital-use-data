...
codelist = read.csv(here("data/meds_covid_meds.csv"))
codes = codelist$id
codes = paste(codes) # convert to strings (they are stored as strings in SCMD table)

#codes <- toString(codelist$id)
```
...
db_scmd_covid <- db_scmd %>% 
  dplyr::filter(vmp_snomed_code %in% codes)

db_scmd_covid <- dplyr::collect(db_scmd_covid)

# df_scmd_covid %>% skimr::skim()

# Tidy tidy tidy data
df_scmd_covid_names <- db_scmd_covid %>% 
...
check_missing <- select(df_scmd_covid_names,c("ods_code", "ods_name"))
check_missing <- check_missing %>% distinct(.keep_all = TRUE)
...
df_tab_sparkline <- df_scmd_covid_names %>% 
...
# SNOMED
vmp_snomed_names_lookup <- df_scmd_covid_names %>% 
  select(vmp_snomed_code, vmp_product_name) %>% 
  dplyr::distinct()

# Trust
trust_names_lookup <- df_scmd_covid_names %>% 
...
df_scmd_covid_mg <- df_scmd_covid_names %>% 
...
df_scmd_covid_ddd <- df_scmd_covid_mg %>% 
...
```{r}
df_scmd_covid_ddd_map <- df_scmd_covid_ddd %>% 
...
temp_ggplot <- df_scmd_covid_ddd_map %>%
...
temp_ggplot <- df_scmd_covid_ddd_map %>% 
  ...
  labs(subtitle = paste0("From: ", min(df_scmd_covid_ddd_map$year_month), " to ", max(df_scmd_covid_ddd_map$year_month)),
       x = NULL,
       y = "Defined Daily Dose",
       fill = NULL) +
...
df_scmd_covid_ddd_map_temp <- df_scmd_covid_ddd_map %>% 
...
temp_ggplot <- df_scmd_covid_ddd_map_temp %>% 
  filter(rank <= 20) %>% 
  ggplot(aes(stp_name)) +
  geom_bar(aes(y = volume_ddd,
               fill = vtmnm,
...
df_scmd_covid_ddd_map_temp_temp <- df_scmd_covid_ddd_map_temp %>% 
...
data_tab <- df_scmd_covid_ddd_map_temp_temp %>% 
  mutate(prop_scale = seq(from = 0, to = 1, length = nrow(df_scmd_covid_ddd_map_temp_temp)))
...
reactable(data_tab, 
          ...
          columns = list(
            stp_name = colDef(name = "STP", 
                              minWidth = 300),
            prop_use_Tocilizumab = colDef(name = "Tocilizumab", 
